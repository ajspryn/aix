            ...
            <div class="flex items-center justify-end gap-3 w-1/4">
                @auth
                    @if(Auth::user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}" class="hidden lg:block text-xs font-bold uppercase tracking-widest text-gray-600 hover:text-gray-900 border border-gray-200 hover:border-gray-300 hover:bg-white px-4 py-2.5 rounded-full transition-all shadow-sm">Command Center</a>
                    @endif
                ...
                @else
                    <a href="{{ route('login') }}" class="bg-gray-900 hover:bg-black text-white px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all shadow-lg hover:shadow-xl hover:shadow-gray-400/30 hover:-translate-y-0.5 border border-transparent flex items-center gap-2 group">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="pt-8 border-t border-zinc-800/50 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="text-zinc-500 text-xs text-center md:text-left tracking-wide">
                    &copy; {{ date('Y') }} AIX Anticipatory Impact Exchange.<br class="md:hidden"/> Hak cipta dilindungi undang-undang keamanan siber.
                </div>
                <!-- Social Connectors -->
