...
<div class="bg-gray-100 min-h-screen pb-28">
    ...
    {{-- ================================================================== --}}
    {{-- HERO SECTION — starts from very top, back button lives INSIDE it   --}}
    {{-- ================================================================== --}}
    <div class="relative w-full overflow-hidden bg-zinc-950" style="height: 70vh; min-height: 480px;">
        {{-- Foto Latar --}}
        <img src="{{ $heroImage }}" alt="Foto Wilayah"
             class="absolute inset-0 w-full h-full object-cover opacity-50">
        {{-- Gradasi bawah --}}
        <div class="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/40 to-transparent"></div>
        {{-- Gradasi atas (untuk memblend nav) --}}
            ...
            <div>
                <div class="flex flex-wrap items-center gap-2.5 mb-5">
                    <span class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-rose-500 text-white rounded-full text-[11px] font-black uppercase tracking-widest border border-rose-400 shadow-[0_0_20px_rgba(239,68,68,0.4)]">
                        ...
                        DARURAT TINGKAT {{ $contract->financial_tier }}
                    </span>
                    <span class="px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full text-[11px] font-bold uppercase tracking-widest">
                        {{ strtoupper($contract->monitoredArea->type) }}
                    </span>
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                        ...
                        <p class="text-2xl font-black text-gray-900 font-mono">{{ number_format($contract->monitoredArea->base_population) }}</p>
                    ...
                    <div class="bg-rose-50 rounded-2xl p-5 border border-rose-100 shadow-sm">
                        ...
                        <p class="text-2xl font-black text-rose-600 font-mono">{{ $contract->monitoredArea->current_risk_score * 100 }}%</p>
                ...
                <div class="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div class="p-8 lg:p-10">
                            ...
                            <p class="text-gray-700 leading-relaxed text-[15px]">{{ $contract->narrative['kondisi_saat_ini'] }}</p>
                        </div>
                        {{-- Parameter Sensor --}}
                        <div class="p-8 lg:p-10 bg-gray-50/60">
                            ...
                            <div class="space-y-3">
                                @foreach($contract->narrative['parameter_risiko'] as $param)
                                @php $isDanger = in_array($param['status'], ['Berbahaya', 'Kritis Ekstrem', 'Anomali', 'Kritis']); @endphp
                                <div class="flex items-center justify-between bg-white p-4 rounded-xl border {{ $isDanger ? 'border-rose-100' : 'border-gray-100' }}">
                                    <span class="text-xs font-bold text-gray-500 uppercase tracking-wide">{{ $param['nama'] }}</span>
                                    <div class="text-right">
                                        <div class="text-base font-black font-mono text-gray-900">{{ $param['nilai'] }}</div>
                                        <div class="text-[9px] font-bold uppercase tracking-widest {{ $isDanger ? 'text-rose-500' : 'text-emerald-500' }}">{{ $param['status'] }}</div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div class="bg-rose-50 p-8 rounded-3xl border border-rose-100">
                        ...
                        <p class="text-rose-900/80 leading-relaxed text-[14px]">{{ $contract->narrative['skenario_risiko'] }}</p>
                    </div>
                    <div class="bg-emerald-50 p-8 rounded-3xl border border-emerald-100">
                        ...
                        <p class="text-emerald-900/80 leading-relaxed text-[14px]">{{ $contract->narrative['proposal_mitigasi'] }}</p>
                    </div>
                </div>
                ...
                {{-- BUKTI VISUAL --}}
                @if(count($contract->monitoredArea->attachments) > 0)
                <div class="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($contract->monitoredArea->attachments as $att)
                        <div class="aspect-video rounded-xl overflow-hidden bg-gray-100 relative group cursor-zoom-in border border-gray-100">
                            <img src="{{ $att->image_url }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                            <div class="absolute inset-0 bg-gradient-to-t from-zinc-950/80 to-transparent flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <span class="text-[9px] text-white/70 font-mono mb-0.5">VISUAL_LOG::{{ $loop->iteration }}</span>
                                <p class="text-[10px] text-white font-medium line-clamp-2">{{ $att->description }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                {{-- SENTIMEN MEDIA & BERITA --}}
                @if(count($sentimentSignals) > 0)
                <div class="bg-indigo-50/50 p-8 rounded-3xl border border-indigo-100 shadow-sm">
                    ...
                    <div class="space-y-4">
                        @foreach($sentimentSignals as $signal)
                        <div class="bg-white p-5 rounded-2xl border border-indigo-50 shadow-sm hover:shadow-md transition">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <span class="text-[10px] text-gray-400 font-mono">{{ $signal->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-[13px] text-gray-700 leading-relaxed italic border-l-2 border-indigo-300 pl-4 py-1">
                                "{{ $signal->raw_text }}"
                            </p>
                            <div class="mt-4 flex items-center justify-between">
                                <div class="text-[10px] font-bold uppercase tracking-widest {{ $signal->calculated_sentiment < -0.5 ? 'text-rose-500' : ($signal->calculated_sentiment > 0.5 ? 'text-emerald-500' : 'text-amber-500') }}">
                                    Sentimen: {{ $signal->calculated_sentiment < -0.5 ? 'Krisis' : ($signal->calculated_sentiment > 0.5 ? 'Positif' : 'Waspada') }} ({{ round($signal->calculated_sentiment * 100) }}%)
                                </div>
                                @if($signal->source_link)
                                <a href="{{ $signal->source_link }}" target="_blank" class="text-[10px] text-indigo-600 hover:text-indigo-800 font-bold uppercase tracking-widest flex items-center gap-1 group">
                                    Sumber Bacaan 
                    ...
                    @if($sentimentSignals->hasPages())
                    <div class="mt-6 pt-4 border-t border-indigo-100/50">
                        {{ $sentimentSignals->links() }}
                    </div>
                    @endif
                </div>
                @endif
                ...
                <div class="sticky top-28 space-y-6">
                    <div class="bg-white rounded-3xl p-8 border border-gray-200 shadow-sm">
                        ...
                        <div class="pb-6 border-b border-gray-100 mb-6">
                            ...
                            <div class="text-3xl font-black text-gray-900 font-mono leading-none">
                                <span class="text-sm text-gray-400 font-sans align-top">IDR </span>{{ number_format($contract->estimated_funding_needed, 0) }}
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            <div class="flex justify-between items-center mb-2">
                                <div>
                                    ...
                                    <p class="text-base font-black text-indigo-600 font-mono">IDR {{ number_format($contract->current_pooled_funds, 0) }}</p>
                                </div>
                                <span class="text-sm font-black text-white bg-indigo-600 px-3 py-1 rounded-full">{{ $contract->progress_percent }}%</span>
                            </div>
                            <div class="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-full bg-indigo-600 rounded-full" style="width: {{ $contract->progress_percent }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6 pb-6 border-b border-gray-100">
                            ...
                            <div class="space-y-2.5">
                                @foreach($contract->narrative['rab_real'] as $rab)
                                <div class="flex items-center gap-2">
                                    <span class="shrink-0 text-[10px] font-black text-indigo-500 bg-indigo-50 border border-indigo-100 w-10 text-center py-0.5 rounded-md">{{ $rab['percent'] }}%</span>
                                    <span class="text-xs text-gray-600 flex-1 truncate">{{ $rab['title'] }}</span>
                                    <span class="shrink-0 text-xs font-bold font-mono text-gray-900">Rp{{ number_format($rab['amount'], 0) }}</span>
                                </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        @endif
                        @if(session('error_donation'))
                            <div class="mb-4 p-4 bg-rose-50 text-rose-700 text-xs font-bold rounded-2xl border border-rose-200">
                                {{ session('error_donation') }}
                            </div>
                        @endif

                        <form action="{{ route('public.contract.donate', $contract->id) }}" method="POST" class="space-y-3">
                            @csrf
                            <div>
                    ...
                    <div class="bg-zinc-950 rounded-3xl p-6 border border-zinc-800 shadow-xl">
                        ...
                        <div class="font-mono text-[11px] space-y-3 h-52 overflow-y-auto">
                            @forelse($contract->monitoredArea->telemetryLogs->sortByDesc('created_at')->take(10) as $log)
                            <div>
                                <span class="text-zinc-600 text-[10px]">[{{ $log->created_at->format('Y-m-d H:i') }}]</span>
                                <div class="mt-0.5 text-zinc-300">
                                    <span class="text-indigo-400">INFO</span>
                                    · RN:<span class="text-white">{{ $log->rainfall_mm }}mm</span>
                                    · TM:<span class="text-white">{{ $log->temperature }}°C</span>
                                    · SM:<span class="text-white">{{ $log->soil_moisture }}%</span>
                                </div>
                            </div>
                            @empty
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const lat = {{ $contract->monitoredArea->latitude }};
        const lng = {{ $contract->monitoredArea->longitude }};
        const riskScore = {{ $contract->monitoredArea->current_risk_score * 100 }};
        const areaName = `{{ $contract->monitoredArea->name }}`;
...
@php $snapToken = session()->pull('snap_token'); @endphp
    ...
    document.addEventListener("DOMContentLoaded", function() {
        snap.pay('{{ $snapToken }}', {
            onSuccess: function(result){
                ...
                form.method = 'POST';
                form.action = '{{ route('public.contract.success', $contract->id) }}';
