...
<div class="p-8">
    ...
    <div class="grid grid-cols-3 gap-6 mb-8">
        <div class="glass-panel p-6 rounded-2xl relative overflow-hidden">
            ...
            <div class="text-4xl font-bold text-white"><?php echo e(number_format($metrics['aggregate_national_risk_score'], 2)); ?></div>
        ...
        <div class="glass-panel p-6 rounded-2xl relative overflow-hidden">
            ...
            <div class="text-4xl font-bold text-emerald-400">IDR <?php echo e(number_format($metrics['cumulative_intake'], 0)); ?></div>
        </div>
        <div class="glass-panel p-6 rounded-2xl relative overflow-hidden">
            <div class="text-xs text-zinc-500 uppercase tracking-widest mb-2">Tingkat Penyaluran Bantuan</div>
            <div class="text-4xl font-bold text-sky-400"><?php echo e($metrics['mitigation_success_rate']); ?>%</div>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-2xl border border-zinc-800 overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-zinc-800 bg-zinc-900/50 flex justify-between items-center">
            ...
            <span class="bg-rose-500/20 text-rose-400 text-xs px-2 py-1 rounded font-bold"><?php echo e($pendingContracts->total()); ?> Diperlukan</span>
        </div>
            ...
            <tbody class="divide-y divide-zinc-800/50">
                <?php $__empty_1 = true; $__currentLoopData = $pendingContracts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contract): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-zinc-800/20 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="font-bold text-zinc-200"><?php echo e($contract->monitoredArea ? $contract->monitoredArea->name : 'Unknown'); ?></div>
                            <div class="text-zinc-600 text-xs">Anomali <?php echo e($contract->monitoredArea ? $contract->monitoredArea->type : 'Unknown'); ?></div>
                        </td>
                        <td class="px-6 py-4 tracking-widest text-xs">
                            <span class="px-2 py-1 rounded bg-zinc-800 text-zinc-300 border border-zinc-700">T-<?php echo e($contract->financial_tier); ?></span>
                        </td>
                        <td class="px-6 py-4 font-mono text-xs text-zinc-400">IDR <?php echo e(number_format($contract->estimated_funding_needed, 0)); ?></td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="<?php echo e(route('admin.contracts.show', $contract->id)); ?>" class="inline-block bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2 rounded font-bold text-xs uppercase tracking-widest shadow-[0_0_15px_rgba(0,0,0,0.3)] hover:scale-[1.01] active:scale-[0.99] border border-zinc-700 transition-all">
                                Detail
                            </a>
                            <?php if($contract->status === 'pending'): ?>
                                <form action="<?php echo e(route('admin.contracts.start', $contract->id)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
        ...
        <?php if($pendingContracts->hasPages()): ?>
        <div class="px-6 py-4 border-t border-zinc-800 bg-zinc-900/50">
            <?php echo e($pendingContracts->links()); ?>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const telemetryData = <?php echo json_encode($recentTelemetry->sortBy('id')->values(), 15, 512) ?>;
    ...
    setTimeout(() => {
        ...
        const areas = <?php echo json_encode($areas, 15, 512) ?>;
...
<?php echo $__env->make('admin.layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ajspryn/Project/aix/resources/views/admin/dashboard.blade.php ENDPATH**/ ?>